test_that("Okabe Ito colors work", {
  ...
  vdiffr::expect_doppelganger(
    "okabeito_fill",
    plot2 + scale_fill_okabeito(order = 7:8, use_black = TRUE)
  )
  vdiffr::expect_doppelganger(
    "okabeito_darken",
    plot2 + scale_fill_okabeito(order = 5:6, darken = 0.2)
  )
  vdiffr::expect_doppelganger(
    "okabeito_lighten",
    plot2 + scale_fill_okabeito(order = 3:4, darken = -0.2)
  )
  vdiffr::expect_doppelganger(
    "okabeito_alpha",
    plot2 + scale_fill_okabeito(order = c(3, 6), alpha = 0.2)
  )
