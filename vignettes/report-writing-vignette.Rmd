...
functions allow for the indefinite article to be added without changing the word to a number. This can be done with `indefinite(8, words = FALSE)`. The fourth and final function is the 
`as.roman` function, which is technically a part of the utils package, which 
means it should be called using `utils::as.roman`. The `as.roman` function 
converts numbers to their Roman numeral equivalent (e.g., 3 becomes III).
