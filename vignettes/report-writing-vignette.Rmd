...
Tables can be created using the kableExtra package in R. If the kableExtra package is not already installed, please install it using the following command:

```{r install_kable, echo = TRUE, eval = FALSE}
install.packages("kableExtra")
```

And then load the package using the following command:

```{r load_kable, echo = TRUE, eval = FALSE}
library(kableExtra)
```

Creating tables is relatively simple. The kable package takes an existing dataset and places the vectors (i.e., columns) into columns in the table. Table formatting can then be specified either within the kable command or within kable_styling function. 

Take the *iris* dataset that is automatically loaded in R. To make a more reasonably sized table for this example, let's filter the dataset to flowers with petal length greater than 6.0 with the following code:
...
Now that the data is more manageable, the smaller dataset can be turned into a table. The full help page can be found by using the `?kable` command. Briefly, *x* holds the data frame, *digits* indicates the number of digits displayed to the right of the decimal point, *col.names* indicates what each column should be called, *align* specifies how to align the data in each column, *caption* specifies the title for the table, *booktabs* specifies formatting for the table (i.e., the bolded column headers, the horizontal lines, the subtle horizontal lines between each row), and *format.args* specifies format arguments to be used in the table. In this instance, *big.mark = ","* specifies that commas should be inserted to the left of the decimal point when necessary to improve readability (i.e., printing 3,400 instead of 3400).

Tables may be referenced using the following inline command: `\@ref(tab:chunk-label)`. Please note, however, that the text to the right of the colon specifies that name of the code chunk. Also note that cross-references cannot use an underscore in the name of the code chunk. The underscore is a special character in Latex, which causes difficulties in references table code chunks with an underscore in the name.
...
In practice, we can then reference the numbered table using `\@ref(tab:iris)`.
