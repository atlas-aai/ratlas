...
library(systemfonts)
require_font("Roboto Condensed", fallback = "sans")
...
Finally, the font has been changed to Roboto Condensed, which is freely available from [Google Fonts](https://fonts.google.com/specimen/Roboto+Condensed).

## Changing the font

Although Roboto Condensed provides a modern font that works in most cases, it is often desirable to have a font that matches the brand requirements.
...
To make the formatting of plots easier, you can use `ggplot2::set_theme()` which will change the ggplot2 defaults so that `theme_atlas()` is always used, and the default color scales are changed to the Okabe Ito palette for discrete scales and the viridis palette for continuous scales.
That is, once `ggplot2::set_theme()` has been called, the `scale_color_okabeito()` and `theme_atlas()` lines no longer need to be added.
...
`ggplot2::set_theme()` also provides flexibility for which defaults are set.
For example, to use the Palatino font by default, use `set_theme(theme_atlas(base_family = "Palatino"))`.
