...
The ATLAS **ggplot2** themes use different fonts from the default theme in Figure \@ref(fig:init-plot). In order for **ggplot2** to be able to use other fonts on your computer, we need to use the **extrafont** package.
...
This will import all TrueType fonts (`.ttf`) into a font database that can be accessed by **ggplot2**. If you later install new fonts to your system, you will need to call `extrafont::font_import()` again to add the new fonts to the database. To use the fonts, you simply have to load the database when you start your *R* session.
...
Alternatively, you can set a global option in your `.Rprofile` to always load the font database when you attach the **ratlas** package with `library(ratlas)`. To edit your `.Rprofile`, use `usethis::edit_r_profile()`. This will open your `.Rprofile`, which is used to set global options. An example `.Rprofile` might look like this:
...
Here, two options have been set. The first defines which CRAN repository to use when installing new packages. The second indicates that the font database should be loaded when **ratlas** is attached. We recommend setting this option in your `.Rprofile` so that you don't have to always remember to load the font database manually.
...
Although Arial Narrow provides a modern font available on all systems, it is often desirable to have a font that matches the brand requirements. The official font of the University of Kansas and ATLAS is [Gotham](https://brand.ku.edu/guidelines/typography). However, this font is expensive, and thus inaccessible to most users. [Montserrat](https://fonts.google.com/specimen/Montserrat) is a very similar font that is available for free through Google Fonts. We recommend that you download and install this font on your system directly from Google Fonts, and then calling `extrafont::font_import()`. However, a convenience function has been provided, `ratlas::import_montserrat()`, that will try do some of the work for you.

Once the Montserrat font has been installed on your system, you can apply it to the ATLAS theme using `ratlas::theme_atlas_ms()`, as shown in Figure \@ref(fig:theme-atlas-ms-ex).

(ref:theme-atlas-ms-ex-cap) Using the Montserrat font with `ratlas::theme_atlas_ms()`.

```{r theme-atlas-ms-ex, fig.cap = "(ref:theme-atlas-ms-ex-cap)"}
ggplot(mtcars, aes(x = mpg, y = disp)) +
  geom_point(aes(color = factor(cyl))) +
  theme_atlas_ms()
```

Notice that Figure \@ref(fig:theme-atlas-ms-ex) is identical to Figure \@ref(fig:theme-atlas-ex), with the exception of a different font (i.e., all other theme changes are preserved).
...
The `set_theme()` functions provides flexibility for which defaults are set. For example, to use the Montserrat font by default, use `set_theme(font = "Montserrat")`. See `?set_theme()` for all options.

## Saving plots

Finally, it is often useful to save plots to a file. This can be done with `ratlas::ggsave2()`. This is a wrapper around `ggplot2::ggsave()`. The `ggsave2()` version applies some defaults to all ATLAS. Specifically, the plot labels and titles are automatically spell checked, the aspect ratio of plots is set to a reasonable default (rather than using the current size of your current graphics device), and embeds the fonts when saving to a PDF format, allowing the plots to render correctly on any system.
